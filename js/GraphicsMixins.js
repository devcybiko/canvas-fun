const GraphicsMixin = {
    ...
    ellipse(args) {
        ...
        this.rect(args.x, args.y, args.w, args.h);
        if (args.fillColor) {
            this._ctx.beginPath();
    ...
    text(args) {
        let defaults = { text: String, x: Number, y: Number, w: 0, h: 0, color: this._color || "black", fillColor: this._fillColor || "", borderColor: this._borderColor || "", font: this._font || "sans-serif", pitch: this._pitch || 12, textAlign: this._textAlign || "center", textBaseline: this._textBaseline || "middle" };
        args = Mixin.getArgs(arguments, defaults);
        if (args.w === 0) args.w = Math.floor(args.text.length * args.pitch * 0.5);
